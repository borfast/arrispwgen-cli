...
function printUsageAndExit(exitCode?: number) {
    ...
    console.log(
        'If you only want the password for one specific date, specify only "start_date".',
    );
    console.log(
        'If "end_date" is also passed, a password for each day between "start_date" and "end_date" will be generated.',
    );
    console.log(
        'The --seed parameter allows you to use a custom seed for the password generator. A seed must be 8 to 10 characters long.',
    );
    console.log(
...
if (dates.length === 1) {
    ...
    try {
        ...
        if (e instanceof arrispwgen.InvalidDateRangeError) {
            console.log(
                'The given dates are out of order. Your start date is after the end date.',
            );
            printUsageAndExit();
        } else {
